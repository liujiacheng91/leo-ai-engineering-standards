...
> This file is auto-loaded by Claude Code when the ll-standards plugin is active.
> It is also used by `scripts/install_project.*` as `.claude/ll-rules.md` for fallback environments.
> Content must stay in sync with `ll-rules.md`. When editing, update both files.
> 
